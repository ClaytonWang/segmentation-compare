...
from .. import db
from ..models import File, Query, Comparison, Marklogic, QueryPlans, Survey,SurveyScore
...
def query():
    if request.method == 'POST':
        ...
        query = Query.query.filter_by(
            body=querystring, engine_id=engine_id).first()
        result = None
        if query and query.segmentation is not None:
            ...
            query = Query(body=querystring, segmentation=result,
                          engine_id=engine_id, created_at=datetime.utcnow())
    ...
    if len(request.files) > 0:
        ...
        if file and allowed_file(file.filename):
            ...
            for row in read_file(filepath):
                query = Query.query.filter_by(
                    body=row[0], engine_id=engine_id).first()
                if not query:
                    query = Query(
                        body=row[0], engine_id=engine_id, created_at=datetime.utcnow())
                    db.session.add(query)
...
def comparison():
    comparisons = Comparison.query.filter_by(
        deleted=0).order_by(Comparison.id.desc()).all()
    ...
    if request.method == 'POST':
        ...
        if len(request.files) == 2:
            ...
            if auto_file and allowed_file(auto_file.filename) and manual_file and allowed_file(manual_file.filename):
                auto_new_filename, auto_filepath = save_file(
                    auto_file, UPLOAD_COMPARISON_FILEPATH)
                manual_new_filename, manual_filepath = save_file(
                    manual_file, UPLOAD_COMPARISON_FILEPATH)
                ...
                flash(
                    {'status': 'failed', 'text': 'Please select the correct file type.'})
        else:
...
def report(report_id):
    comparison_data = Comparison.query.filter_by(
        id=report_id, deleted=0).first()
...
def phraselist():
    marklogics = Marklogic.query.filter_by(
        deleted=0).order_by(Marklogic.id.desc()).all()
...
def queryplan():
    marklogics = Marklogic.query.filter_by(
        deleted=0, status=2).order_by(Marklogic.id.desc()).all()
    phraselistnames = [item.phraselist_name for item in marklogics]
    phraselistnames = list(set(phraselistnames))

    history = QueryPlans.query.filter_by(
        deleted=0).order_by(QueryPlans.id.desc()).all()
...
def queryplan_crud():
    if request.method == 'POST':
        ...
        try:
            ...
            template = handler.get_insert_queryplan_template(
                hlct, phraselist_name)
        ...
        if len(request.files) > 0:
            ...
            if file and allowed_file(file.filename):
                ...
                if querySetName is None and querySetName == '':
                    flash(
                        {'status': 'failed', 'text': 'Please enter a QuerySetName.'})
                ...
                filepath = os.path.join(
                    main.root_path, DOWNLOAD_FILEPATH, filename)
                print data
...
def compare_data(auto_file_data, manual_file_data):
    ...
    for row in auto_file_data:
        ...
        if segmentation != None:
            item = {'id': index, 'query': row[0], 'auto_segmentation': row[1],
                    'manual_segmentation': segmentation[0], 'score': segmentation[1]}
...
def normalize_segmentation(str):
    """1. AB => ab
       ...
       5. '   ' => ' '"""
    str = str.strip().lower()
    str = re.sub(r'\[([a-z0-9]+ [a-z0-9 ]+)\]', lambda x: '[' +
                 '] | ['.join(x.group(1).split(' ')) + ']', str)
